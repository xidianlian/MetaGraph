# -*- coding: utf-8 -*-
"""
Created on Wed Mar 20 14:37:51 2019
...
train_kernel_path = OUTPUT_PATH + "\\meta3"
test_kernel_path = OUTPUT_PATH + "\\meta3.test"
def get_data():
...
def get_train_kernel(train_data, api_method):
#    app-api-pack-api-app 和 app-perm-app 元图构建的核矩阵
    app_api_mat = train_data[:, 0:400]
    app_perm_mat = train_data[:, 400:704]
    
    app_api_method = np.dot(app_api_mat, api_method)
    app_api_method_api = np.dot(app_api_method, api_method.T)
    app_api_method_api_app = np.dot(app_api_method_api, app_api_mat.T)
    
    app_perm_app = np.dot(app_perm_mat, app_perm_mat.T)
    
    app_app = app_api_method_api_app * app_perm_app
    
    output_file(app_app, train_kernel_path)
    return app_app # shape(16645, 16645)

def get_test_kernel(train_data, test_data, api_method):
...
#   testapp-api-method-api-trainapp
    app_api_method = np.dot(test_app_api_mat, api_method)
    app_api_method_api = np.dot(app_api_method, api_method.T)
    app_api_method_api_app = np.dot(app_api_method_api, train_app_api_mat.T)
    
    app_perm_app = np.dot(test_app_perm_mat, train_app_perm_mat.T)
    
    app_app = app_api_method_api_app * app_perm_app # shape(4162, 16645)
...
if __name__ == "__main__":
    method_api = pd.read_csv(api_method_path, dtype=np.int32) # shape(400, 301)
    method_api = method_api.as_matrix()
    api_method = method_api.T;
    
    X_train, X_test, Y_train, Y_test =get_data()
    train_kernel = get_train_kernel(X_train, api_method)
    test_kernel = get_test_kernel(X_train, X_test, api_method)
