# -*- coding: utf-8 -*-
"""
Created on Tue Mar 19 14:17:31 2019
...
train_kernel_path = OUTPUT_PATH + "\\meta1"
test_kernel_path = OUTPUT_PATH + "\\meta1.test"
train_label_path = OUTPUT_PATH + "\\y_train"
test_label_path = OUTPUT_PATH + "\\y_test"

def get_data():
    app_api = pd.read_csv(app_api_path ,dtype=np.uint8) # shape(20807, 400)
    app_perm = pd.read_csv(app_perm_path ,dtype=np.uint8)# shape(20807, 304)
...
def get_train_kernel(train_data):
#    app-api-app 和 app-perm-app 元图构建的核矩阵
    app_api_mat = train_data[:, 0:400]
    app_perm_mat = train_data[:, 400:704]
    app_api_app = np.dot(app_api_mat, app_api_mat.T)
    app_perm_app = np.dot(app_perm_mat, app_perm_mat.T)
    app_app = app_api_app * app_perm_app
    
    output_file(app_app, train_kernel_path)
    
    return app_app # shape(16645, 16645)
def get_test_kernel(train_data, test_data):
    ...
    app_api_app = np.dot(test_app_api_mat, train_app_api_mat.T)
    app_perm_app = np.dot(test_app_perm_mat, train_app_perm_mat.T)
    
    app_app = app_api_app * app_perm_app # shape(4162, 16645)
...
if __name__ == "__main__":
    ...
    train_kernel = get_train_kernel(X_train)
    test_kernel = get_test_kernel(X_train, X_test)
#    np.savetxt(train_label_path, Y_train, fmt='%d')
#    np.savetxt(test_label_path, Y_test, fmt='%d')
